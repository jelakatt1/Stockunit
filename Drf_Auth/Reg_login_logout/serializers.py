...
class UploadFileSerializer(serializers.ModelSerializer):
    ...
    def create_product(self, validated_data):
        info = stripe.Product.create(name=validated_data.get('title'))
        price_info = stripe.Price.create(
            unit_amount=validated_data.get('price') * 100,
            currency="usd",
            product=info.get('id'),
        )
        return price_info
    ...
    def create(self, validated_data):
        price_info = self.create_product(validated_data)
        file = UploadFile.objects.create(**validated_data, stripe_product_info=price_info)
        file.save()
        return file
